...
## Purpose: Main descriptive analysis
## QA: Matthew Barclay
## ---------------------------------

# Initial set-up ------
source("./scripts/common_scripts/00_base_setup.R")
current_script <- "Main-2_descriptive.R"
...
numb_tumours_site |> write.csv(str_glue("{results_loc}numb_tumours_site_{run_date}.csv"), row.names = F)
...
n_pat_by_tumour_numb |> write.csv(str_glue("{results_loc}n_pat_by_tumour_numb_{run_date}.csv"), row.names = F)
...
## Suppress values
summary_all_suppress <- summary_all |> group_by(var) |> suppress_values_with_prop(n_var = n_patients)
...
# Find frequency of number of presenting symptoms per tumour ----
...
n_symptoms_by_sex |> write.csv(str_glue("{results_loc}n_symptoms_by_sex_{run_date}.csv"), row.names = F)
...
numb_elig_symptoms |> write.csv(str_glue("{results_loc}numb_elig_symptoms_{run_date}.csv"), row.names = F)
## No need to suppress values
